...
class CouchDocumentStore implements IDocumentStore
{
    ...
    public function nextUuid($prefix = '')
    {
        $uuids = $this->client->nextUuids();
    ...
    public function save(IDocument $document)
    {
        ...
        if (! isset($data[self::COUCH_ID]))
        {
            ...
            $uid = $this->nextUuid($document->getIdentifierPrefix());
            $data[self::COUCH_ID] = $uid;
            $document->setIdentifier($uid);
        }
        ...
        else if('conflict' === $result['error'])
        {
            // update the document's revision and retry.
            $document->setRevision(
                $this->client->statDoc(NULL, $document->getIdentifier())
            );
            $data = self::prepareStorageData($document);
            $result = $this->client->storeDoc(NULL, $data);
        }
    ...
    public function bulkSave(array $documents)
    {
        ...
        foreach ($documents as $document)
        {
            ...
            {
                throw new Exception(
                    "Invalid object type passed to bulkSave invocation. Only IDocument instances allowed."
                );
            }
        ...
        $retryData = array();
        // @todo set uuids to be consistent when creating entries per bulk.
        foreach ($this->client->storeDocs(NULL, $data) as $docResult)
        {
            if(isset($docResult['error']) && 'conflict' === $docResult['error'])
            {
                $document = $documentIdMap[$docResult['id']];
                // update the document's revision and retry.
                $document->setRevision(
                    $this->client->statDoc(NULL, $document->getIdentifier())
                );
                $retryData[] = self::prepareStorageData($document);
            }
        }

        if (! empty($retryData))
        {
            foreach ($this->client->storeDocs(NULL, $retryData) as $docResult)
            {
                if(isset($docResult['error']) && 'conflict' === $docResult['error'])
                {
                    return FALSE;
