...
/**
 * The RssDataSource class is a concrete implementation of the BaseDataSource base class.
...
class RssDataSource extends BaseDataSource
{
     ...
     * @return      boolean
     *
     * @see         BaseDataSource::forwardCursor()
     */
    ...
    protected function loadFeedByUrl($feedUrl)
    {
        ...
        $curlHandle = ProjectCurl::create($feedUrl);
        ...
        if (! curl_exec($curlHandle) || 200 != curl_getinfo($curlHandle, CURLINFO_HTTP_CODE))
        {
            ...
            throw new DataSourceException(
                sprintf(
                    'Curl failed for url: "%s", Curl error: %s@%s ',
                    $feedUrl,
    ...
    /**
     * (non-PHPdoc)
     * @see BaseDataSource::getCurrentOrigin()
     */
