 ...
 * @subpackage      Mvc
 */
class Shofi_Verticals_ListAction extends ShofiVerticalsBaseAction
{
    /**
    ...
    public function executeRead(AgaviRequestDataHolder $parameters)
    {
        ...
        $listState = ListState::fromArray(array(
            ...
            'limit' => $parameters->getParameter('limit', 50),
    ...
    protected function prepareListData(array $items, IWorkflowService $workflowService)
    {
        ...
        foreach ($items as $workflowItem)
        {
            ...
            if (! $ticket)
            {
                error_log(__METHOD__ . " - Missing ticket for (vertical)workflow-item: " . $workflowItem->getIdentifier());
                continue;
            }
            $ticketData = array(
