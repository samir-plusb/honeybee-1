...
/**
 * The WorkflowBaseInteractivePlugin serves as the base for interactive plugins.
...
abstract class WorkflowBaseInteractivePlugin extends WorkflowBasePlugin
{
    ...
    protected abstract function getPluginResource();
    ...
    /**
     * Returns whether the plugin is executable at the it's current state.
     *
     * @return boolean
     */
    protected function mayProcess()
    {
        if (($user = $this->ticket->getSessionUser()))
        {
            $operation = sprintf(
                '%s::%s',
                $this->getPluginId(),
