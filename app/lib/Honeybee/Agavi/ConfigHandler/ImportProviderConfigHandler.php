...
class ImportProviderConfigHandler extends BaseConfigHandler
{
    /**
     * Holds the name of the data_source document schema namespace.
     */
    const XML_NAMESPACE = 'http://berlinonline.de/schemas/honeybee/config/provider/definition/1.0';
    ...
    {
        $document->setDefaultNamespace(self::XML_NAMESPACE, 'provider');
        ...
        foreach ($document->getConfigurationElements() as $cfgNode)
        {
            $parsedSourceDefinitions = array_merge_recursive(
                ...
                $this->parseSourceDefinitions(
                    $cfgNode->getChild('providers')
                )
            );
        }

        $data = array('providers' => $parsedSourceDefinitions);
    ...
    /**
     * Parse the given providers node and return the corresponding array representation.
     *
    ...
    protected function parseSourceDefinitions(\AgaviXmlConfigDomElement $dataSourcesElement)
    {
        ...
        foreach ($dataSourcesElement->getChildren('provider') as $dataSourceElement)
        {
