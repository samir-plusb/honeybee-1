...
class Message implements IMail
{
    ...
    /**
     * @var boolean Set this to true if validation should silently ignore invalid email addresses instead of throwing exceptions
     */
    ...
    /**
     * @return array[] of attachments with each attachment array having the following keys: content, name, content_type and content_disposition
     */
    ...
    protected function getConsolidated($addresses, $only_return_first = false)
    {
        if (empty($addresses) || (!is_string($addresses) && !is_array($addresses)))
        {
            ...
            throw new MessageConfigurationException('The given argument is not a valid email address. Please use either a string with a valid email address or give an associative array with keys being the email address and value being the display name. You may omit the display name or set it to null if you want.');
        }
        ...
        if (is_string($addresses))
        {
            ...
            else
            {
                ...
                throw new MessageConfigurationException('The given argument "' . $addresses . '"is not a valid email address. Please use either a string with a valid email address or give an associative array with keys being the email address and value being the display name. You may omit the display name or set it to null if you want.');
            }
        }
        ...
        foreach ($addresses as $email => $display_name)
        {
            ...
            else
            {
                if (!$this->shouldIgnoreValidationErrors())
                {
                    throw new MessageConfigurationException('The given array has a key "' . $email . '" that is not a valid email address.');
                }
            }
        }
     ...
     * @return array
     */
    protected function createAttachment($content, $name, $content_type, $content_disposition, $type = self::ATTACHMENT_TYPE_DATA)
    {
        if (!empty($name))
        {
            $name = trim($name);
