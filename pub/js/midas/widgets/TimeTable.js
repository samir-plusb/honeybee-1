midas.widgets.TimeTable = midas.widgets.Widget.extend({
        ...
        if (this.options.data)
        {
            for (var i = 0; i < this.options.data.length; i++)
            {
                ...
                this.time_spans.push(
                    new midas.widgets.TimeTable.TimeSpan(timespan.from, timespan.to)
                );
            }
        }
    },
    ...
    {
        return midas.widgets.TimeTable.TPL;
    },
    ...
    addCurrentTimeSpan: function()
    {
        ...
        this.time_spans.push(new midas.widgets.TimeTable.TimeSpan(from, to));
...
// #     nested classes     #
// ##########################
midas.widgets.TimeTable.TimeSpan = midas.core.BaseObject.extend({
...
// #     constants     #
// #####################
midas.widgets.TimeTable.DEFAULT_OPTIONS = {
    autobind: true,
    fieldname: 'timetable',
...
midas.widgets.TimeTable.TPL = ''
+'<div class="span3">' // ** input time: from-section (select day & enter time) **
+'  <div class="control-group">'
+'      <label>Von</label>'
+'      <div class="controls">'
+'          <select class="tagslist-from-day" '
+'                  data-bind="options: days_of_week, '
+'                             selectedOptions: input_from.day, '
+'                             event: { keypress: onInput }">'
+'          </select>'
+'          <input class="tagslist-from-time" placeholder="09:00" '
...
+'      <label>Bis</label>'
+'      <div class="controls">'
+'          <select class="tagslist-to-day" '
+'                  data-bind="options: days_of_week, '
+'                             selectedOptions: input_to.day, '
+'                             event: { keypress: onInput }">'
+'          </select>'
+'          <input class="tagslist-to-time" placeholder="09:00"'
